...
def test_after_insert_oai_set(app,db):
    with patch("invenio_oaiserver.receivers._new_percolator"):
...
def test_after_update_oai_set(app,db):
    class Target:
        @property
        def search_pattern(self):
            return "test_search_pattern"
        ...
        def spec(self):
            return "test_spec"
    with patch("invenio_oaiserver.receivers._delete_percolator"):
        with patch("invenio_oaiserver.receivers._new_percolator"):
            # with patch("invenio_oaiserver.receivers.update_affected_records.delay"):
...
def test_after_delete_oai_set(app,db):
    class Target:
        @property
        def search_pattern(self):
            return "test_search_pattern"
