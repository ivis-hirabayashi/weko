...
def test_generate_random_str():
    ...
    mock_choice = MagicMock(side_effect=lambda x: x[0])  # Always return the first character
    rng.choice = mock_choice

    with patch("weko_accounts.utils.random.SystemRandom", return_value=rng):
        result = generate_random_str(10)
        assert result == random_list[0] * 10  # The expected value is repeated 10 times by the same character.
        mock_choice.assert_has_calls([mock.call(random_list) for _ in range(10)])
...
def test_login_required_customize(app,users):
    ...
    with patch("weko_accounts.utils.current_app.login_manager._login_disabled",return_value=True):
        with app.test_request_context(method="GET"):
            result = login_required_customize(lambda x,y:x+y)(x=1,y=2)
            assert result == 3
...
def test_roles_required(app,users):
    ...
    with app.test_request_context(method="GET"):
        ...
        with patch("weko_accounts.utils.abort",return_value=make_response()) as mock_abort:
            result = roles_required(roles)(lambda x,y:x+y)(x=1,y=2)
